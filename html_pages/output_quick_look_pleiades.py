...
def parse_args():
    ...
    parser.add_argument('quick_out',type=str,nargs=1)
...
def make_quicklook_page(ds,dirout,plotnames):
    # put some stuff on an html page
    outfilename = dirout+"/"+ds.basename + "_quicklook.html"
...
def look_at_output(clobber,dir_out,quick_out):
...
##Check to see if the plots exist or force a remake
	...
	plotnames = [density_string,temperature_string,metallicity_string,HI_string]
        ...
        if clobber:
            make_quicklook_page(ds,QUICK_DIR,plotnames)
...
if __name__ == "__main__":
    ...
    quick_out = args.quick_out[0]

    print "dir_out",dir_out
    print "quick_out",quick_out
    message = look_at_output(clobber,dir_out,quick_out)
    sys.exit(message)
